...
export const mocVendors = mysqlTable(
	...
	(table) => [primaryKey({ columns: [table.id], name: "moc_vendors_id" })],
);
...
export const mocs = mysqlTable(
	...
	(table) => [primaryKey({ columns: [table.id], name: "mocs_id" })],
);

export const mstCity = mysqlTable(
	"mst_city",
	{
		id: int().autoincrement().notNull(),
...
export const mstItemCategories = mysqlTable(
	"mst_item_categories",
	{
		id: int().autoincrement().notNull(),
...
export const mstVendors = mysqlTable(
	"mst_vendors",
	{
		id: int().autoincrement().notNull(),
		name: varchar({ length: 255 }).notNull(),
	...
	(table) => [primaryKey({ columns: [table.id], name: "mst_vendors_id" })],
);
...
export const roleModules = mysqlTable(
	...
	(table) => [
		...
		unique("role_modules_type_slug_unique").on(
			table.userType,
			table.moduleSlug,
		),
	],
...
export const users = mysqlTable(
	"users",
	{
		...
		status: mysqlEnum(["Contract", "Permanent", "Intern", "Leave"])
			.default("Contract")
			.notNull(),
...
export const vesselItemStandard = mysqlTable(
...
export const vesselRequestItems = mysqlTable(
	"vessel_request_items",
	{
		...
		unit: mysqlEnum(["Pcs", "Box", "Liter", "Meter", "Kg"]).notNull(),
		status: mysqlEnum(["Approved by system", "Waiting", "Approved", "Rejected"])
			.default("Waiting")
			.notNull(),
...
export const vesselRequests = mysqlTable(
	"vessel_requests",
	{
		...
		vesselId: int("vessel_id")
			.notNull()
			.references(() => mstVessels.id),
		status: mysqlEnum(["Ok", "Waiting", "Approved", "Rejected"])
			.default("Waiting")
			.notNull(),
		priority: mysqlEnum(["Low", "Medium", "High"]).default("Medium").notNull(),
		justification: text(),
