import { int, mysqlTable, mysqlEnum, timestamp, varchar, decimal, text, index, foreignKey } from 'drizzle-orm/mysql-core';
...
export const poStatusEnum = ['Auto Approved', 'Pending Approval', 'Approved', 'Rejected'] as const;
...
export const purchaseOrders = mysqlTable(
  'purchase_orders',
  {
    ...
    mocId: int('moc_id').notNull().references(() => mocs.id),
    ...
    status: mysqlEnum('status', poStatusEnum).default('Pending Approval').notNull(),
    notes: text('notes'),
