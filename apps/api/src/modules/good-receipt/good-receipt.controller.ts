...
const getGoodReceipts = asyncHandler(async (req: Request, res: Response) => {
  ...
  const result = await grRepo.getGoodReceipts(Number(page), Number(limit), search, status);
...
const getPendingPOsForReceipt = asyncHandler(async (_req: Request, res: Response) => {
  try {
    const pos = await grRepo.getPendingPOsForReceipt();
    return success(res, { purchaseOrders: pos });
  } catch (err: any) {
    throw new AppError(err.message || 'Failed to fetch pending purchase orders', 400);
  }
});
