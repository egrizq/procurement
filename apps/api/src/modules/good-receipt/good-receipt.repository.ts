	...
	async createGoodReceipt(data: {
		...
		const po = await db.query.purchaseOrders.findFirst({
			...
			with: {
				vesselRequestItem: {
					with: {
						item: { columns: { id: true } },
					},
				},
				moc: {
					with: {
						vesselRequest: { columns: { vesselId: true } },
					},
				},
			},
		});
		...
		await db.transaction(async (tx) => {
			const [inserted] = await tx.insert(goodReceipts).values({
				grNumber,
			...
			if (status === "Accepted") {
				const vesselId = po.moc?.vesselRequest?.vesselId;
				const itemId = po.vesselRequestItem?.itemId;

				if (!vesselId || !itemId) {
					throw new Error(
						"Could not resolve Kapal or Item details from Purchase Order",
					);
				}

				const stock = await tx.query.vesselStocks.findFirst({
					where: and(
						eq(vesselStocks.vesselId, vesselId),
						eq(vesselStocks.itemId, itemId),
					),
				});
				...
				if (stock) {
					await tx
						.update(vesselStocks)
						.set({
							stockOnHand: stock.stockOnHand + po.qty,
							lastUpdate: new Date(),
							updatedAt: new Date(),
						})
						.where(eq(vesselStocks.id, stock.id));
				} else {
					await tx.insert(vesselStocks).values({
						vesselId,
						itemId,
						stockOnHand: po.qty,
						lastUpdate: new Date(),
		...
		return await this.getGoodReceiptById(insertedId!);
	}
			...
			with: {
				purchaseOrder: {
					with: {
						...
						vesselRequestItem: {
							with: {
								item: { columns: { id: true, name: true, itemCode: true } },
							},
						},
						moc: {
							with: {
	...
	async getGoodReceipts(
		...
		if (search) {
			const results = await db
				...
				.leftJoin(
					purchaseOrders,
					eq(goodReceipts.purchaseOrderId, purchaseOrders.id),
				)
				.leftJoin(mstVendors, eq(purchaseOrders.vendorId, mstVendors.id))
				.leftJoin(
					vesselRequestItems,
					eq(purchaseOrders.vesselRequestItemId, vesselRequestItems.id),
				)
		...
		const conditions = [];
		if (matchingIds !== null)
			conditions.push(inArray(goodReceipts.id, matchingIds));
