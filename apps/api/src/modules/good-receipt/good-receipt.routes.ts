...
router.post('/list', roleAuth(), validate(goodReceiptListSchema), controller.getGoodReceipts);
router.get('/po-list', roleAuth(), controller.getPendingPOsForReceipt);
router.post('/', roleAuth(), validate(createGoodReceiptSchema), controller.createGoodReceipt);
router.get('/:id', roleAuth(), validate(goodReceiptByIdSchema), controller.getGoodReceiptById);
