...
class MstVendorRepository {
  ...
  async addMstVendor({
    ...
    const [vendor] = await db.select().from(mstVendors).where(eq(mstVendors.id, inserted[0].insertId)).limit(1);
    return vendor;
  }
  ...
  async updateMstVendor(
    ...
    let [vendor] = await db.select().from(mstVendors).where(eq(mstVendors.id, id)).limit(1);
    return vendor;
  }
