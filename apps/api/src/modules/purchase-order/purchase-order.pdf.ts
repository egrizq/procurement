	...
	const tableBody: any[] = [
		...
		[
			{ text: "1", alignment: "center", fillColor: "#ffffff" },
			{
				text: [
					{
						text: po.vesselRequestItem?.item?.name || "Unknown Item",
						bold: true,
					},
			...
			{
				text: `${po.qty} ${po.vesselRequestItem?.unit || "Pcs"}`,
				alignment: "center",
				fillColor: "#ffffff",
			},
			{
				text: formatPrice(po.unitPrice),
				alignment: "right",
				fillColor: "#ffffff",
			},
			{
				text: formatPrice(po.totalAmount),
				alignment: "right",
				fillColor: "#ffffff",
			},
		],
		...
		content: [
			...
			// Metadata Block
			{
				stack: [
					...
					{
						text: `PO Date: ${formatDate(po.createdAt)}`,
						margin: [0, 0, 0, 4],
					},
					{
						text: `MOC Ref Code: ${po.moc?.vesselRequest?.requestCode || "-"}`,
						margin: [0, 0, 0, 4],
					},
					{
						text: `Vessel: ${po.moc?.vesselRequest?.vessel?.name || "-"}`,
						margin: [0, 0, 0, 0],
					},
				],
			...
			// Vendor Details
			{
				stack: [
					...
					{
						text: `Vendor Name: ${po.vendor?.name || "-"}`,
						bold: true,
						fontSize: 11,
					},
				],
				...
				columns: [
					{
						...
						stack: [
							...
							{
								text: po.createdByUser?.fullName || "Staff / Buyer",
								fontSize: 10,
								alignment: "center",
								bold: true,
							},
							{
								text: "Date: " + formatDate(po.createdAt),
								fontSize: 9,
								alignment: "center",
								color: "#6b7280",
							},
