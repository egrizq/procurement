import { z } from 'zod';
import { usersTypeEnum, departmentEnum, userStatusEnum } from '../../../db/schema/enums.ts';
...
export const getUsersSchema = z.object({
	body: z.object({
		...
		department: z.enum(departmentEnum, { message: 'Invalid department' }).optional(),
...
export const createUserSchema = z.object({
	body: z.object({
		username: z
			...
			.regex(/^[a-zA-Z0-9_-]+$/, 'Username must contain only alphanumeric characters, hyphens, or underscores'),
		email: z
		...
		vesselId: z.number('Vessel is required').int().positive('Vessel ID must be a positive integer'),
		position: z.string().max(100, 'Position must not exceed 100 characters').optional(),
		status: z.enum(userStatusEnum, { message: 'Invalid status' }).optional(),
		imgUrl: z.string().max(255, 'Image URL must not exceed 255 characters').optional(),
	}),
});
...
export const updateUserSchema = z.object({
	...
	body: z.object({
		...
		type: z.enum(usersTypeEnum, { message: 'Invalid user type' }).optional(),
		department: z.enum(departmentEnum, { message: 'Invalid department' }).optional(),
		vesselId: z.number().int().positive('Vessel ID must be a positive integer').optional(),
		position: z.string().max(100, 'Position must not exceed 100 characters').optional(),
		status: z.enum(userStatusEnum, { message: 'Invalid status' }).optional(),
