...
class VesselItemStandardRepository {
	async getStandards(
		...
		if (search) {
			const results = await db
				.select({ id: vesselItemStandards.id })
		...
		const condition = matchingIds
			? inArray(vesselItemStandards.id, matchingIds)
			: undefined;
	...
	async update(
		id: number,
		data: Partial<typeof vesselItemStandards.$inferInsert>,
	) {
		await db
			.update(vesselItemStandards)
			.set(data)
			.where(eq(vesselItemStandards.id, id));
