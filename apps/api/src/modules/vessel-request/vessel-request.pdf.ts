...
export async function generateVesselRequestPdf(request: any): Promise<Buffer> {
	...
	items.forEach((item: any, idx: number) => {
		...
		tableBody.push([
			...
			{
				text: item.qtyApproved !== null ? item.qtyApproved.toString() : "-",
				alignment: "center",
				fillColor: rowBg,
			},
		...
		content: [
			...
			// Metadata Block
			{
				stack: [
					...
					{
						text: `Vessel: ${request.vessel?.name || "-"}`,
						margin: [0, 0, 0, 4],
						bold: true,
					},
				...
				columns: [
					{
						...
						stack: [
							...
							{
								text: "Date: " + formatDate(request.requestDate),
								fontSize: 9,
								alignment: "center",
								color: "#6b7280",
							},
						...
						stack: [
							...
							},
							{
								text:
									request.status === "Approved"
										...
										: request.status === "Approved by system"
											? "Approved by System"
											: "Pending Review",
								fontSize: 10,
								alignment: "center",
							...
							},
							{
								text:
									...
									request.status === "Approved by system"
										? "Date: " +
											formatDate(request.reviewedAt || request.requestDate)
