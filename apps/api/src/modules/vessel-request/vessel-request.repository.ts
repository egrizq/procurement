...
class VesselRequestRepository {
	...
	async getVesselRequests(
		...
		if (status) {
			if (status === "Approved") {
				conditions.push(
					inArray(vesselRequests.status, ["Approved", "Approved by system"]),
				);
			} else {
		...
		const items = itemsResult.map((item) => {
			const availableItems = item.items.filter(
				(i: any) =>
					(i.status === "Approved" ||
						i.status === "Approved by system" ||
						(i.qtyApproved && i.qtyApproved > 0)) &&
	...
	async findRecentRequestedItems(
		vesselId: number,
		itemIds: number[],
		days = 30,
	) {
		...
		const results = await db
			...
			.innerJoin(
				vesselRequests,
				eq(vesselRequestItems.vesselRequestId, vesselRequests.id),
			)
			.where(
				and(
	...
	async reviewRequest(
		...
		await db.transaction(async (tx) => {
			...
			const currentItems = await tx
				.select()
				.from(vesselRequestItems)
				.where(eq(vesselRequestItems.vesselRequestId, id));
			...
			for (const item of currentItems) {
				...
				if (action === "Approve" && itemsAdjustment) {
					const adjustment = itemsAdjustment.find(
						(adj) => adj.itemId === item.itemId,
					);
					if (adjustment) {
