      ...
      <form @submit.prevent="handleSubmit" class="space-y-4">
        ...
        <div>
          ...
          <select
            ...
            required
            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white text-sm"
          >
        ...
        <!-- Selected PO Details -->
        <div v-if="selectedPO" class="space-y-4 border-t border-gray-200 pt-4">
          <h4 class="text-sm font-medium text-gray-700">Detail Purchase Order</h4>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Nomor PO</label>
              <input :value="selectedPO.poNumber" type="text" disabled class="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-100 cursor-not-allowed" />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Status</label>
              <input :value="selectedPO.status" type="text" disabled class="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-100 cursor-not-allowed" />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Kapal / Request Code</label>
              <input :value="`${selectedPO.moc?.vesselRequest?.vessel?.name || '-'} / ${selectedPO.moc?.vesselRequest?.requestCode || '-'}`" type="text" disabled class="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-100 cursor-not-allowed" />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Vendor</label>
              <input :value="selectedPO.vendor?.name || '-'" type="text" disabled class="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-100 cursor-not-allowed" />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Item</label>
              <input :value="`${selectedPO.vesselRequestItem?.item?.name || '-'} (${selectedPO.vesselRequestItem?.item?.itemCode || '-'})`" type="text" disabled class="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-100 cursor-not-allowed" />
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1">Jumlah / Harga Satuan</label>
              <input :value="`${selectedPO.qty} ${selectedPO.vesselRequestItem?.unit || 'Pcs'} / Rp ${formatNumber(selectedPO.unitPrice)}`" type="text" disabled class="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-100 cursor-not-allowed" />
            </div>
          </div>
          <div v-if="selectedPO.notes">
            <label class="block text-sm font-medium text-gray-700 mb-1">Catatan PO</label>
            <textarea :value="selectedPO.notes" rows="2" disabled class="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-100 cursor-not-allowed"></textarea>
          </div>
          ...
          <div class="border-t border-gray-200 pt-4 space-y-4">
            ...
            <div v-if="!grForm.isSameItem" class="space-y-1.5">
              ...
              <textarea
                ...
                required
                rows="3"
                class="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500 bg-white"
            ...
            <div class="space-y-2 pt-4 border-t border-gray-200">
              ...
              <div
                class="border-2 border-dashed border-gray-300 hover:border-indigo-500 rounded-lg p-5 transition-colors duration-150 flex flex-col items-center justify-center cursor-pointer bg-gray-50 hover:bg-indigo-50/30"
              ...
              <div v-if="selectedFiles.length > 0" class="mt-3 space-y-2">
                <div
                  ...
                  :key="idx"
                  class="flex items-center justify-between p-2.5 bg-white border border-gray-300 rounded-lg"
                >
